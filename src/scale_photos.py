...
def scale_folder(folder):
    scaled = os.path.join(folder,'scaled')
    os.mkdir(scaled)
    files = [file for file in os.listdir(folder) if JPGPAT.match(file)]
    for file in files:
        sfile = os.path.join(folder,file)
        dfile = os.path.join(scaled, file)
        print(f"{sfile} -> {dfile}")
        resize_image(sfile, dfile, [2000, 2000], quality=85)

def parse_args():
    """Parse the args."""
    parser = argparse.ArgumentParser(
        description='Index Photos into a folder structure')
    parser.add_argument('-i','--inputFolder', type=str, required=False,
                        default='sandbox',
